...
#ifdef YAPB_INCLUDED
   // added additions (for bot cvars). (dz)
   inline bool GetBool (void) { return value > 0.0; }
   inline int GetInt (void) { return static_cast <int> (value); }
   inline int GetFlags (void) { return flags; }
   inline float GetFloat (void) { return value; }
   inline char *GetString (void) { return strval; }
...
typedef struct
{
   ...
   void      (*pfnSetupVisibility) (struct edict_s *pViewEntity, struct edict_s *pClient, unsigned char **pvs, unsigned char **ucPAS);
